...
class TemTemporal__features(nn.Module):
    ...
    def forward(self, x_real, x_img):
        b = x_real.shape[0]
        x_real += self.Temporal_pos_embed
        x_img += self.Temporal_pos_embed
        x_real, x_img = self.pos_drop(x_real), self.pos_drop(x_img)
        x, temp = self.block1(x_real, x_img)
        x, temp = self.block2(x, temp)
        x, temp = self.block3(x, temp)
        x, temp = self.block4(x, temp)
